...
def compute_loci_statistics_custom_reciprocal(data, column="query_locus", order_by="best_bitscore"):
    """
    For each query_locus, return summary metrics
    :param data: modin.pandas.dataframe object, results against rvdb database 
                                                includes family and kingdom of rvdb hits
    ...
    data["virus_hit"] = data["all_taxonomy"].astype(str).str.contains("virus", case=False)
    data["retro_hit"] = data["all_taxonomy"].astype(str).str.contains("retro|caulimo|pararna", case=False)

    # Best title per locus
    best_titles = data.loc[data.groupby(column)["bitscore"].idxmax()][[column, "cleaned_title", "hit_length", "family"]]
    best_titles = best_titles.rename(columns={
        "cleaned_title": "best_title_rvdb",
        "hit_length": "best_hit_len", 
        "family": "best_hit_fam"
    })
    ...
    stats = data.groupby(column).agg(
        ...
        n_retros=("retro_hit", "sum"),
        rvdb_fams=("family", lambda x: "|".join(x.dropna().unique())),
        rvdb_kings=("viral_kingdom", lambda x: "|".join(x.dropna().unique()))
    ).reset_index()
...
def classify_from_rvdb_only(df, retro_prop = 0.7, no_viral = 0.3, rvdb_nr_eve_prop = 0.6,):
    ...
    def classify(row):
        # === Precompute all needed fields ===
        n_hits_rvdb = row.get("n_hits", 0)
        n_retros = row.get("n_retros", 0)
        n_virus_hits_rvdb = row.get("n_virus_hits", 0)
        rvdb_kings = str(row.get("rvdb_kings", "")).lower()
        rvdb_fams = str(row.get("rvdb_fams", "")).lower()

        title_all = " ".join([
            str(row.get("best_title", "")),
        ...
        is_likely_retro = (
            n_hits_rvdb > 0 and
            n_retros > 0 and
            n_retros / n_hits_rvdb > retro_prop and
            re.search(r"gag|pol|env|reverse|pro[- ]?pol|protease|pro|rt[- ]?in", title_all)
        )
        ...
        is_transposon = (
            n_hits_rvdb > 0 and
            n_virus_hits_rvdb / n_hits_rvdb < no_viral and
            re.search(r"\bpol\b(?!yprotein)|transpos|gag|group specific antigen", best_title)
        )
...
def full_reciprocal_results(reciprocal_rvdb, reciprocal_nr, hits_taxonomy, old_version = False):
    """
    Load results when HI-FEVER was used against nr and rvdb databases   
    """
    ...
    reciprocal_columns = [
     ...
     # === Load reciprocal rvdb results
    d_rvdb = pd.read_csv(reciprocal_rvdb, sep="\t", header=None, names=reciprocal_columns)
    d_rvdb["cleaned_title"] = d_rvdb["hit_title"].apply(clean_subject_title)

    tax = pd.read_csv(hits_taxonomy, sep="\t", header=None,
    ...
    d_nr = pd.read_csv(reciprocal_nr, sep="\t", header=None, names=reciprocal_columns)
    ...
    # Get hit statistics for each loci
    loci_stats_nr = compute_loci_statistics_nr(d_nr)

    # Classify results by comparing nr and rvdb statistics
    both_dbs = loci_stats_nr.merge(loci_stats_rvdb, on="query_locus", how="outer", suffixes=("_nr", "_rvdb"))
    res = classify_query_loci(both_dbs)
    return(res)

def custom_reciprocal_results(reciprocal_rvdb, hits_taxonomy):
    """
    Load results when HI-FEVER was used against custom reciprocal database   
    """
    # === Load reciprocal data
    reciprocal_columns = [
        'query_locus', 'subject_protein', 'percent_identity', 'hit_length', 'mismatches', 'gapopens',
        'query_start', 'query_end', 'subject_start', 'subject_end', 'e_value', 'bitscore',
        'hit_title', 'full_sequence'
    ]

    # === Load reciprocal rvdb results
    d_rvdb = pd.read_csv(reciprocal_rvdb, sep="\t", header=None, names=reciprocal_columns)
    d_rvdb["protein_accn"] = d_rvdb["hit_title"].str.extract(r"^([A-Z]{2,}(?:_[\d]+|[\d]+)(?:\.\d+)?)")
    d_rvdb["cleaned_title"] = d_rvdb["hit_title"].str.extract(r"\|([^|]+)$")

    # === Load taxonomy
    tax = pd.read_csv(hits_taxonomy, sep="\t")
    
    d_rvdb_t = d_rvdb.merge(tax, left_on="protein_accn", right_on="record_id", how="left")
    d_rvdb_t["family"] = d_rvdb_t["family"].astype(str)
    d_rvdb_t = d_rvdb_t.sort_values(by="family", kind="mergesort", ignore_index=True)

    # === Calculate statistics
    loci_stats_rvdb = compute_loci_statistics_custom_reciprocal(d_rvdb_t)

    # === Clasif hits
    res = classify_from_rvdb_only(loci_stats_rvdb)
    return(res)
...
def parse_arguments():
    ...
    parser.add_argument("-of", "--out_folder", type=str, help= "Folder to store output tables")

    # Custom reciprocal or full reciprocal (both nr and rvdb)
    parser.add_argument("-cr", "--custom_reciprocal", action='store_true', help="Results come from a custom reciprocal search")
    ...
    if args.folder:
        ...
        # Reciprocal results
        if args.custom_reciprocal:
            args.reciprocal_rvdb = args.reciprocal_rvdb or resolve_path_or_error(args.folder, "reciprocal-matches.dmnd.tsv")
        ...
        # Rest of the inputs are common to both
        ...
        args.folder = os.path.dirname(args.reciprocal_rvdb)

    if not args.out_folder:
        args.out_folder = args.folder
...
def main():
    ...
    if args.custom_reciprocal:
        res = custom_reciprocal_results(args.reciprocal_rvdb, args.taxonomy)
        
    else: 
        res = full_reciprocal_results(args.reciprocal_rvdb, args.reciprocal_nr, args.taxonomy, args.old_version)
    ...
    # Merge host metadata and genewise results
    ...
    output_prefix = os.path.join(args.out_folder, args.label)
